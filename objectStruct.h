...
/**
 * @struct MemoryNode
 * @brief Represents a memory entity with attributes such as name, type, offset, and value.
 */
struct MemoryNode {
     ...
     */
    MemoryNode(std::string nodeName, int nodeType, int nodeOffset, int nodeValue)
        : name(std::move(nodeName)), type(nodeType), offset(nodeOffset), value(nodeValue) {}
 ...
 */
struct UsageInfo {
    int usageStatus;  ///< Indicates if the variable is used (-1 for unset, 1 for used, 0 for not used).
    int lifetime;     ///< Indicates the lifetime status (-1 for unset, 1 for active, 0 for inactive).
