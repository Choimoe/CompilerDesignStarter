...
std::string Lexer::removeComments(std::string str) {
    ...
    while (str.find("/*") != std::string::npos) {
        ...
        str.erase(pos, pos1 - pos + 2); // Erase the comment part (including the comment markers)
    }
    ...
    while (str.find("//") != std::string::npos) {
        ...
        str.erase(pos, pos1 - pos + 1); // Erase the comment from the start to the end
    }
    return str;
}

// This function adds spaces around operators in the input string to separate them
// from adjacent tokens, making it easier for tokenization.
std::string Lexer::insertSpaces(std::string str) {
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (i + 2 <= n && OpKeyMap::operators.count(str.substr(i, 2)))
            s.append(" ").append(str.substr(i++, 2)).append(" "); // Add a space around the operator
        // Check if the current character is an operator
        else if (i + 1 <= n && OpKeyMap::operators.count(str.substr(i, 1)))
            s.append(" ").append(1, str[i]).append(" "); // Add a space around the operator
        else
            s.append(1, str[i]); // Otherwise, just append the character as it is
    }
    return s;
}

// This function processes an identifier (e.g., a variable or function name) from the string.
// It assumes the identifier starts at the given index.
std::vector<std::string> Lexer::processIdentifier(std::string& token,
    ...
    std::string identifier(
        str_view.substr(idx, end - (str_view.begin() + idx))); // Extract the identifier
    if (!identifier.empty()) {
        ans.push_back(identifier + " IDENT\n"); // Add the identifier with "IDENT" token type
        idx += identifier.size(); // Move the index forward by the length of the identifier
    }
    return ans;
}

// This function processes a number (integer or floating-point) from the string.
// It starts at the given index and returns a vector containing the tokenized number.
std::vector<std::string> Lexer::processNumber(std::string& token, size_t& idx) {
    ...
    for (auto it = str_view.begin() + idx;
        ...
        h.push_back(*it);
        if (*it == '.') {
            isFloat = true; // Mark as floating-point number if decimal point is found
        }
    }
...
// This function processes a keyword (e.g., "if", "else", "while") from the string.
// If the token matches a known keyword, it returns the keyword with its token type.
...
// The main lexer function that processes the input string by first removing comments,
// adding spaces around operators, and then breaking the string into tokens.
